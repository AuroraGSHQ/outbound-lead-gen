...
not homeowners: Apollo.io (the lead-sourcing data source) is a B2B contact
...
python scripts/run_sourcing_once.py       # sanity-check Apollo.io + ICP config
...
Full setup (Google Cloud OAuth client, Apollo.io key, Calendly webhook,
...
- **Desktop (Chrome/Edge):** an install icon appears in the address bar, or
  use the browser menu → "Install Cosmos."
...
Thirty-one agents, each backed by a real scheduled job (`app/scheduler.py`)
or run on-demand — see them, their cadence, and their last-run status on the
**Team** page once the app is running. Every agent that could produce
something client-facing (an email, an ad, outreach copy) queues it in
**Approvals** or the **Actions** board rather than sending/publishing it —
see Architecture below.

Agents are named after things in the sky, not a finite pantheon — the
company is the universe, so the naming pool is effectively unlimited,
which matters once you're past two dozen of them.
...
|---|---|---|
| **Voyager** | Sources leads from Apollo.io against your ICP daily; Vibe Prospecting sourcing is a person-run request/import loop (see below) | — |
| **Spectrum** | Passive checks on prospect sites (speed, mobile, tracking, click-to-call); flags what a human still has to verify | §8 |
| **Beacon** | Drafts first-touch/follow-up emails, polls replies, classifies intent. Also texts and calls leads (Twilio + ElevenLabs) once they have a phone number on file | §9 |
| **Horizon** | Turns discovery-call notes into a client record + action plan | new |
| **Constellation** | Watches for 90-day reviews and drafts the referral ask | §11 |
| **Orbit** | Generates the monthly ad campaign brief from the budget calendar | §4/§6/§14 |
| **Observatory** | Computes the six KPIs daily; drafts the quarterly benchmark extract | §12/§15 |
| **Relay** | Daily digest + immediate pings (meeting booked, etc.) | — |
| **Launchpad** | Plants the onboarding checklist the moment a client goes active | new |
| **Equinox** | Flags monthly invoices due and escalates unconfirmed ones | new |
| **Radiance** | Nudges for reviews at 60 days; drafts a reply for any review you log | new |
| **Zenith** | Drafts a case study once a client has 90 days of real results | §1 |
| **Telescope** | On-demand pre-call brief on a prospect, before a discovery call | new |
| **Axis** | Turns an intake recommendation into a ready-to-send proposal | new |
| **Sentinel** | Watches send-volume trend and Gmail token health | new |
| **Eclipse** | Logs a competitor ad, checks it against Aurora's differentiation test | new |
| **Gravity** | Six weeks after a decline, drafts a low-pressure referral ask | §11 |
| **Prism** | Runs Spectrum's own checks against Aurora's own site | new |
| **Nova** | Flags ad campaigns running past 3-4 weeks for a creative refresh | §6 |
| **Collision** | Flags an active client gone quiet for 60+ days | new |
| **Parallax** | Logs a competitor's pricing, checks it against Aurora's own | new |
| **Epoch** | A once-a-month ops rollup — what's aging, by category | new |
| **Core** | Checks config, dependency drift, and every agent's last run; alerts immediately when something's actually broken | new |
| **Wormhole** | On-demand: paste in a lead's details, get them pushed straight into that client's own CRM (HubSpot, Monday.com, GoHighLevel) | new |
| **Pulsar** | On-demand: drafts organic social post captions for a client's own result | new |
| **Quasar** | On-demand: spots the single best-performing channel from the latest metrics and flags it as worth more budget | new |
| **Comet** | On-demand: flags a KPI trending the same direction several snapshots running | new |
| **Nebula** | On-demand: drafts a blog post / newsletter section from a client's story or the latest metrics | new |
| **Apollo** | On-demand: builds a shot list / production checklist for a campaign's video/photo needs | new |
| **Starlight** | On-demand: reviews a client's brand voice/asset consistency and raises what it finds | new |
| **Supernova** | On-demand: assembles the go-live checklist the moment a campaign is marked ready to publish | new |

### How 31 agents avoid stepping on each other
...
2. **One owner per field.** Each status (`Lead.status`, `Client.status`,
   ...
   Collision (churn) only ever raises a flag — Equinox (billing) or a person
...
3. **Distinct triggers.** Agents watching similar things key off different
   ...
   Gravity's 6-week win-back can't double-ask a client Constellation's
   90-day review already asked.
4. **Read-only by default.** Anything that's "just watching" (Eclipse,
   Parallax, Epoch, Core, Comet) never writes to a core Lead/Client/Message
   table — it only ever creates its own `ActionItem`.

## Sourcing with Vibe Prospecting

Apollo.io sourcing (Voyager) runs on its own every morning, no one has to
touch it. Vibe Prospecting (Explorium data — intent topics, tech stack,
job-change events, richer than Apollo.io's filters) can't run that way: its own rules
...
3. Back on the request's page, upload that CSV. You'll map its columns to
   ...
   Apollo.io leads and enter the same pipeline from there — drafts, approval
...
app/integrations/               → thin clients: Apollo.io, Gmail, Calendly, Claude, site checks, ad platforms
app/services/                   → the 31 agents: sourcing, outreach, conversation, approvals,
                                   ...
                                   agent_toggles, social_posts, campaign_opportunities, trend_watch,
                                   longform_content, production, brand_audit, campaign_launch
...
other agent (Horizon, Spectrum, Gravity...) — funnels through
